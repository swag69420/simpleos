...
a20_waitOutput ;wait until keyboard output buffer isn't empty
    ...
    in al, 0x64
    test al, 1
    jnz a20_waitOutputLoop
    popa
ret

a20_waitInput ;wait until keybord input buffer isn't empty
    pusha
    a20_waitInputLoop:
    in al, 0x64
    test al, 2
...
dapack:
        db 0x10
        db 0
.count: dw 0 ; int 13 resets to # of blocks actually read/written
...
START:
    ...
    mov [boot_disk], d1 ; BIOS fills d1 with disk number
